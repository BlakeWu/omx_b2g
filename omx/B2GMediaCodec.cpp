...
// static
sp<B2GMediaCodec> B2GMediaCodec::CreateByType(
        const sp<ALooper> &looper, const char *mime, bool encoder) {
    sp<B2GMediaCodec> codec = new B2GMediaCodec(looper);
...
// static
sp<B2GMediaCodec> B2GMediaCodec::CreateByComponentName(
...
B2GMediaCodec::B2GMediaCodec(const sp<ALooper> &looper)
    : mState(UNINITIALIZED),
      mLooper(looper),
      mCodec(new B2GACodec),
      mReplyID(0),
      mFlags(0),
      mSoftRenderer(NULL),
...
B2GMediaCodec::~B2GMediaCodec() {
    CHECK_EQ(mState, UNINITIALIZED);
}

// static
status_t B2GMediaCodec::PostAndAwaitResponse(
...
status_t B2GMediaCodec::init(const char *name, bool nameIsType, bool encoder) {
...
status_t B2GMediaCodec::configure(
...
status_t B2GMediaCodec::createInputSurface(
...
status_t B2GMediaCodec::release() {
...
status_t B2GMediaCodec::queueInputBuffer(
        size_t index,
        size_t offset,
        size_t size,
...
status_t B2GMediaCodec::queueSecureInputBuffer(
        size_t index,
        size_t offset,
...
status_t B2GMediaCodec::dequeueInputBuffer(size_t *index, int64_t timeoutUs) {
...
status_t B2GMediaCodec::dequeueOutputBuffer(
        size_t *index,
        size_t *offset,
        size_t *size,
        int64_t *presentationTimeUs,
...
status_t B2GMediaCodec::renderOutputBufferAndRelease(size_t index) {
...
status_t B2GMediaCodec::releaseOutputBuffer(size_t index) {
    sp<AMessage> msg = new AMessage(kWhatReleaseOutputBuffer, id());
    msg->setSize("index", index);
...
status_t B2GMediaCodec::signalEndOfInputStream() {
...
status_t B2GMediaCodec::getOutputFormat(sp<AMessage> *format) const {
...
status_t B2GMediaCodec::getName(AString *name) const {
...
status_t B2GMediaCodec::getInputBuffers(Vector<sp<ABuffer> > *buffers) const {
...
status_t B2GMediaCodec::getOutputBuffers(Vector<sp<BufferInfo> > *buffers) const {
    sp<AMessage> msg = new AMessage(kWhatGetBuffers, id());
...
status_t B2GMediaCodec::requestIDRFrame() {
    (new AMessage(kWhatRequestIDRFrame, id()))->post();

    return OK;
}

void B2GMediaCodec::requestActivityNotification(const sp<AMessage> &notify) {
...
void B2GMediaCodec::cancelPendingDequeueOperations() {
...
bool B2GMediaCodec::handleDequeueInputBuffer(uint32_t replyID, bool newRequest) {
    if (mState != STARTED
...
bool B2GMediaCodec::handleDequeueOutputBuffer(uint32_t replyID, bool newRequest) {
    sp<AMessage> response = new AMessage;
...
void B2GMediaCodec::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatCodecNotify:
        {
            ...
            switch (what) {
                ...
                case B2GACodec::kWhatComponentAllocated:
                {
                ...
                case B2GACodec::kWhatComponentConfigured:
                {
                ...
                case B2GACodec::kWhatInputSurfaceCreated:
                {
                    // response to B2GACodec::kWhatCreateInputSurface
                ...
                case B2GACodec::kWhatSignaledInputEOS:
                {
                    // response to B2GACodec::kWhatSignalEndOfInputStream
                    sp<AMessage> response = new AMessage();
                    status_t err;
                    if (msg->findInt32("err", &err)) {
                        response->setInt32("err", err);
                ...
                case B2GACodec::kWhatBuffersAllocated:
                {
                    int32_t portIndex;
                    ...
                    sp<B2GACodec::PortDescription> portDesc =
                        static_cast<B2GACodec::PortDescription *>(obj.get());
                ...
                case B2GACodec::kWhatOutputFormatChanged:
                {
                ...
                case B2GACodec::kWhatFillThisBuffer:
                {
                ...
                case B2GACodec::kWhatDrainThisBuffer:
                {
                ...
                case B2GACodec::kWhatShutdownCompleted:
                {
                    if (mState == STOPPING) {
                        setState(INITIALIZED);
                    } else {
                ...
                case B2GACodec::kWhatFlushCompleted:
                {
        ...
        case kWhatRelease:
        {
            ...
            if (mState != INITIALIZED
                ...
                // Similarly stopping a stopped B2GMediaCodec should be benign.
                sp<AMessage> response = new AMessage;
                response->setInt32(
                        "err",
        ...
        case kWhatGetBuffers:
        {
            ...
            Vector<sp<BufferInfo> > *dstBuffers;
...
void B2GMediaCodec::extractCSD(const sp<AMessage> &format) {
    mCSD.clear();
...
status_t B2GMediaCodec::queueCSDInputBuffer(size_t bufferIndex) {
    CHECK(!mCSD.empty());
...
void B2GMediaCodec::setState(State newState) {
...
void B2GMediaCodec::returnBuffersToCodec() {
    returnBuffersToCodecOnPort(kPortIndexInput);
    returnBuffersToCodecOnPort(kPortIndexOutput);
}

void B2GMediaCodec::returnBuffersToCodecOnPort(int32_t portIndex) {
...
size_t B2GMediaCodec::updateBuffers(
...
status_t B2GMediaCodec::onQueueInputBuffer(const sp<AMessage> &msg) {
    size_t index;
    size_t offset;
    size_t size;
    int64_t timeUs;
...
status_t B2GMediaCodec::onReleaseOutputBuffer(const sp<AMessage> &msg) {
    size_t index;
    CHECK(msg->findSize("index", &index));
...
ssize_t B2GMediaCodec::dequeuePortBuffer(int32_t portIndex) {
    CHECK(portIndex == kPortIndexInput || portIndex == kPortIndexOutput);
...
status_t B2GMediaCodec::setNativeWindow(
...
void B2GMediaCodec::postActivityNotificationIfPossible() {
...
status_t B2GMediaCodec::setParameters(const sp<AMessage> &params) {
...
status_t B2GMediaCodec::onSetParameters(const sp<AMessage> &params) {
    mCodec->signalSetParameters(params);

    return OK;
}

status_t B2GMediaCodec::amendOutputFormatWithCodecSpecificData(
        const sp<ABuffer> &buffer) {
    AString mime;

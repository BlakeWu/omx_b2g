...
//#define LOG_NDEBUG 0
#define LOG_TAG "B2GACodec"

#include "B2GACodec.h"

#include <binder/MemoryDealer.h>

#include <stagefright/foundation/hexdump.h>
#include <stagefright/foundation/ABuffer.h>
#include <stagefright/foundation/ADebug.h>
#include <stagefright/foundation/AMessage.h>

#include <stagefright/BufferProducerWrapper.h>
#include <stagefright/MediaCodecList.h>
#include <stagefright/MediaDefs.h>
#include <stagefright/NativeWindowWrapper.h>
#include <stagefright/OMXClient.h>
#include <stagefright/OMXCodec.h>
#include <stagefright/ExtendedCodec.h>
...
struct B2GACodec::BaseState : public AState {
    BaseState(B2GACodec *codec, const sp<AState> &parentState = NULL);
...
struct B2GACodec::DeathNotifier : public IBinder::DeathRecipient {
...
struct B2GACodec::UninitializedState : public B2GACodec::BaseState {
    UninitializedState(B2GACodec *codec);
...
struct B2GACodec::LoadedState : public B2GACodec::BaseState {
    LoadedState(B2GACodec *codec);
...
private:
    friend struct B2GACodec::UninitializedState;
...
struct B2GACodec::LoadedToIdleState : public B2GACodec::BaseState {
    LoadedToIdleState(B2GACodec *codec);
...
struct B2GACodec::IdleToExecutingState : public B2GACodec::BaseState {
    IdleToExecutingState(B2GACodec *codec);
...
struct B2GACodec::ExecutingState : public B2GACodec::BaseState {
    ExecutingState(B2GACodec *codec);
...
struct B2GACodec::OutputPortSettingsChangedState : public B2GACodec::BaseState {
    OutputPortSettingsChangedState(B2GACodec *codec);
...
struct B2GACodec::ExecutingToIdleState : public B2GACodec::BaseState {
    ExecutingToIdleState(B2GACodec *codec);
...
struct B2GACodec::IdleToLoadedState : public B2GACodec::BaseState {
    IdleToLoadedState(B2GACodec *codec);
...
struct B2GACodec::FlushingState : public B2GACodec::BaseState {
    FlushingState(B2GACodec *codec);
...
void B2GACodec::setNotificationMessage(const sp<AMessage> &msg) {
    mNotify = msg;
}

void B2GACodec::initiateSetup(const sp<AMessage> &msg) {
    msg->setWhat(kWhatSetup);
    msg->setTarget(id());
    msg->post();
}

void B2GACodec::signalSetParameters(const sp<AMessage> &params) {
    sp<AMessage> msg = new AMessage(kWhatSetParameters, id());
    msg->setMessage("params", params);
    msg->post();
}

void B2GACodec::initiateAllocateComponent(const sp<AMessage> &msg) {
    msg->setWhat(kWhatAllocateComponent);
    msg->setTarget(id());
    msg->post();
}

void B2GACodec::initiateConfigureComponent(const sp<AMessage> &msg) {
    msg->setWhat(kWhatConfigureComponent);
    msg->setTarget(id());
    msg->post();
}

void B2GACodec::initiateCreateInputSurface() {
    (new AMessage(kWhatCreateInputSurface, id()))->post();
}

void B2GACodec::signalEndOfInputStream() {
    (new AMessage(kWhatSignalEndOfInputStream, id()))->post();
}

void B2GACodec::initiateStart() {
    (new AMessage(kWhatStart, id()))->post();
}

void B2GACodec::signalFlush() {
    ALOGV("[%s] signalFlush", mComponentName.c_str());
    (new AMessage(kWhatFlush, id()))->post();
}

void B2GACodec::signalResume() {
    (new AMessage(kWhatResume, id()))->post();
}

void B2GACodec::initiateShutdown(bool keepComponentAllocated) {
    sp<AMessage> msg = new AMessage(kWhatShutdown, id());
    msg->setInt32("keepComponentAllocated", keepComponentAllocated);
    msg->post();
}

void B2GACodec::signalRequestIDRFrame() {
    (new AMessage(kWhatRequestIDRFrame, id()))->post();
}

void B2GACodec::signalConcurrencyParam(bool streamPaused) {
    sp<AMessage> msg = new AMessage(kWhatConcurrencyParam, id());
    msg->setInt32("streamPaused", streamPaused);
    msg->post();
}

status_t B2GACodec::allocateBuffersOnPort(OMX_U32 portIndex) {
    ...
    if (mNativeWindow != NULL && portIndex == kPortIndexOutput) {
        ...
        if (err == OK) {
            ...
            mDealer[portIndex] = new MemoryDealer(totalSize, "B2GACodec");
    ...
    notify->setInt32("what", B2GACodec::kWhatBuffersAllocated);
...
status_t B2GACodec::allocateOutputBuffersFromNativeWindow() {
    OMX_PARAM_PORTDEFINITIONTYPE def;
    InitOMXParams(&def);
...
status_t B2GACodec::cancelBufferToNativeWindow(BufferInfo *info) {
...
B2GACodec::BufferInfo *B2GACodec::dequeueBufferFromNativeWindow() {
    ANativeWindowBuffer *buf;
    int fenceFd = -1;
...
status_t B2GACodec::freeBuffersOnPort(OMX_U32 portIndex) {
...
status_t B2GACodec::freeOutputBuffersNotOwnedByComponent() {
    for (size_t i = mBuffers[kPortIndexOutput].size(); i-- > 0;) {
        BufferInfo *info =
            &mBuffers[kPortIndexOutput].editItemAt(i);
...
status_t B2GACodec::freeBuffer(OMX_U32 portIndex, size_t i) {
...
B2GACodec::BufferInfo *B2GACodec::findBufferByID(
...
status_t B2GACodec::setComponentRole(
...
status_t B2GACodec::configureCodec(
...
status_t B2GACodec::setMinBufferSize(OMX_U32 portIndex, size_t size) {
    OMX_PARAM_PORTDEFINITIONTYPE def;
    InitOMXParams(&def);
    def.nPortIndex = portIndex;
...
status_t B2GACodec::selectAudioPortFormat(
...
status_t B2GACodec::setupAACCodec(
...
status_t B2GACodec::setupAMRCodec(bool encoder, bool isWAMR, int32_t bitrate) {
...
status_t B2GACodec::setupG711Codec(bool encoder, int32_t numChannels) {
...
status_t B2GACodec::setupFlacCodec(
...
status_t B2GACodec::setupRawAudioFormat(
...
status_t B2GACodec::setVideoPortFormatType(
        OMX_U32 portIndex,
...
status_t B2GACodec::setSupportedOutputFormat() {
    OMX_VIDEO_PARAM_PORTFORMATTYPE format;
    InitOMXParams(&format);
...
status_t B2GACodec::setupVideoDecoder(
...
status_t B2GACodec::setupVideoEncoder(const char *mime, const sp<AMessage> &msg) {
    int32_t tmp;
...
status_t B2GACodec::setCyclicIntraMacroblockRefresh(const sp<AMessage> &msg, int32_t mode) {
...
status_t B2GACodec::setupMPEG4EncoderParameters(const sp<AMessage> &msg) {
...
status_t B2GACodec::setupH263EncoderParameters(const sp<AMessage> &msg) {
    int32_t bitrate, iFrameInterval;
    if (!msg->findInt32("bitrate", &bitrate)
            || !msg->findInt32("i-frame-interval", &iFrameInterval)) {
        return INVALID_OPERATION;
...
status_t B2GACodec::setupAVCEncoderParameters(const sp<AMessage> &msg) {
    int32_t bitrate, iFrameInterval;
    if (!msg->findInt32("bitrate", &bitrate)
            || !msg->findInt32("i-frame-interval", &iFrameInterval)) {
        return INVALID_OPERATION;
...
status_t B2GACodec::verifySupportForProfileAndLevel(
...
status_t B2GACodec::configureBitrate(
...
status_t B2GACodec::setupErrorCorrectionParameters() {
...
status_t B2GACodec::setVideoFormatOnPort(
        OMX_U32 portIndex,
...
status_t B2GACodec::initNativeWindow() {
...
size_t B2GACodec::countBuffersOwnedByComponent(OMX_U32 portIndex) const {
    size_t n = 0;
...
size_t B2GACodec::countBuffersOwnedByNativeWindow() const {
    size_t n = 0;
...
void B2GACodec::waitUntilAllPossibleNativeWindowBuffersAreReturnedToUs() {
    if (mNativeWindow == NULL) {
        return;
    }
...
bool B2GACodec::allYourBuffersAreBelongToUs(
        OMX_U32 portIndex) {
    for (size_t i = 0; i < mBuffers[portIndex].size(); ++i) {
        BufferInfo *info = &mBuffers[portIndex].editItemAt(i);
...
void B2GACodec::deferMessage(const sp<AMessage> &msg) {
    bool wasEmptyBefore = mDeferredQueue.empty();
    mDeferredQueue.push_back(msg);
}

void B2GACodec::processDeferredMessages() {
...
void B2GACodec::sendFormatChange(const sp<AMessage> &reply) {
    sp<AMessage> notify = mNotify->dup();
...
void B2GACodec::signalError(OMX_ERRORTYPE error, status_t internalError) {
    sp<AMessage> notify = mNotify->dup();
    notify->setInt32("what", B2GACodec::kWhatError);
    notify->setInt32("omx-error", error);
    notify->setInt32("err", internalError);
    notify->post();
}

status_t B2GACodec::pushBlankBuffersToNativeWindow() {
...
B2GACodec::PortDescription::PortDescription() {
}

status_t B2GACodec::requestIDRFrame() {
    if (!mIsEncoder) {
        return ERROR_UNSUPPORTED;
    }
...
void B2GACodec::PortDescription::addBuffer(
        IOMX::buffer_id id, const sp<ABuffer> &buffer) {
    mBufferIDs.push_back(id);
    mBuffers.push_back(buffer);
}

size_t B2GACodec::PortDescription::countBuffers() {
    return mBufferIDs.size();
}

IOMX::buffer_id B2GACodec::PortDescription::bufferIDAt(size_t index) const {
    return mBufferIDs.itemAt(index);
}

sp<ABuffer> B2GACodec::PortDescription::bufferAt(size_t index) const {
    return mBuffers.itemAt(index);
}

////////////////////////////////////////////////////////////////////////////////

B2GACodec::BaseState::BaseState(B2GACodec *codec, const sp<AState> &parentState)
    : AState(parentState),
      mCodec(codec) {
}

B2GACodec::BaseState::PortMode B2GACodec::BaseState::getPortMode(OMX_U32 portIndex) {
    return KEEP_BUFFERS;
}

bool B2GACodec::BaseState::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        ...
        case B2GACodec::kWhatOMXMessage:
        {
            return onOMXMessage(msg);
        }

        case B2GACodec::kWhatCreateInputSurface:
        case B2GACodec::kWhatSignalEndOfInputStream:
        {
...
bool B2GACodec::BaseState::onOMXMessage(const sp<AMessage> &msg) {
    int32_t type;
...
bool B2GACodec::BaseState::onOMXEvent(
...
bool B2GACodec::BaseState::onOMXEmptyBufferDone(IOMX::buffer_id bufferID) {
...
void B2GACodec::BaseState::postFillThisBuffer(BufferInfo *info) {
    ...
    notify->setInt32("what", B2GACodec::kWhatFillThisBuffer);
...
void B2GACodec::BaseState::onInputBufferFilled(const sp<AMessage> &msg) {
    IOMX::buffer_id bufferID;
...
void B2GACodec::BaseState::getMoreInputDataIfPossible() {
    if (mCodec->mPortEOS[kPortIndexInput]) {
        return;
    }
...
bool B2GACodec::BaseState::onOMXFillBufferDone(
        IOMX::buffer_id bufferID,
        size_t rangeOffset, size_t rangeLength,
        OMX_U32 flags,
        int64_t timeUs,
    ...
    if (index >= 0) {
        B2GACodec::BufferStats *stats = &mCodec->mBufferStats.editValueAt(index);
    ...
    switch (mode) {
        ...
        case RESUBMIT_BUFFERS:
        {
            ...
            sp<AMessage> notify = mCodec->mNotify->dup();
            notify->setInt32("what", B2GACodec::kWhatDrainThisBuffer);
            notify->setPointer("buffer-id", info->mBufferID);
            notify->setBuffer("buffer", info->mData);
            ...
            if (flags & OMX_BUFFERFLAG_EOS) {
                ...
                sp<AMessage> notify = mCodec->mNotify->dup();
                notify->setInt32("what", B2GACodec::kWhatEOS);
...
void B2GACodec::BaseState::onOutputBufferDrained(const sp<AMessage> &msg) {
    IOMX::buffer_id bufferID;
    CHECK(msg->findPointer("buffer-id", &bufferID));
...
B2GACodec::UninitializedState::UninitializedState(B2GACodec *codec)
    : BaseState(codec) {
}

void B2GACodec::UninitializedState::stateEntered() {
...
bool B2GACodec::UninitializedState::onMessageReceived(const sp<AMessage> &msg) {
    bool handled = false;

    switch (msg->what()) {
        case B2GACodec::kWhatSetup:
        {
            onSetup(msg);
        ...
        case B2GACodec::kWhatAllocateComponent:
        {
        ...
        case B2GACodec::kWhatShutdown:
        {
            ...
            sp<AMessage> notify = mCodec->mNotify->dup();
            notify->setInt32("what", B2GACodec::kWhatShutdownCompleted);
            notify->post();
        ...
        case B2GACodec::kWhatFlush:
        {
            sp<AMessage> notify = mCodec->mNotify->dup();
            notify->setInt32("what", B2GACodec::kWhatFlushCompleted);
            notify->post();
...
void B2GACodec::UninitializedState::onSetup(
...
bool B2GACodec::UninitializedState::onAllocateComponent(const sp<AMessage> &msg) {
...
//        androidSetThreadPriority(tid, prevPriority);
    ...
    {
        sp<AMessage> notify = mCodec->mNotify->dup();
        notify->setInt32("what", B2GACodec::kWhatComponentAllocated);
...
B2GACodec::LoadedState::LoadedState(B2GACodec *codec)
    : BaseState(codec) {
}

void B2GACodec::LoadedState::stateEntered() {
...
void B2GACodec::LoadedState::onShutdown(bool keepComponentAllocated) {
...
bool B2GACodec::LoadedState::onMessageReceived(const sp<AMessage> &msg) {
    bool handled = false;

    switch (msg->what()) {
        case B2GACodec::kWhatConfigureComponent:
        {
...
bool B2GACodec::LoadedState::onConfigureComponent(
        const sp<AMessage> &msg) {
    ...
    {
        sp<AMessage> notify = mCodec->mNotify->dup();
        notify->setInt32("what", B2GACodec::kWhatComponentConfigured);
        notify->post();
    }
...
void B2GACodec::LoadedState::onCreateInputSurface(
        const sp<AMessage> &msg) {
    ALOGV("onCreateInputSurface");

    sp<AMessage> notify = mCodec->mNotify->dup();
    notify->setInt32("what", B2GACodec::kWhatInputSurfaceCreated);
...
void B2GACodec::LoadedState::onStart() {
    ALOGV("onStart");
...
B2GACodec::LoadedToIdleState::LoadedToIdleState(B2GACodec *codec)
    : BaseState(codec) {
}

void B2GACodec::LoadedToIdleState::stateEntered() {
...
status_t B2GACodec::LoadedToIdleState::allocateBuffers() {
...
bool B2GACodec::LoadedToIdleState::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatShutdown:
        {
...
bool B2GACodec::LoadedToIdleState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
...
B2GACodec::IdleToExecutingState::IdleToExecutingState(B2GACodec *codec)
    : BaseState(codec) {
}

void B2GACodec::IdleToExecutingState::stateEntered() {
    ALOGV("[%s] Now Idle->Executing", mCodec->mComponentName.c_str());
}

bool B2GACodec::IdleToExecutingState::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatShutdown:
        {
            mCodec->deferMessage(msg);
...
bool B2GACodec::IdleToExecutingState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
        case OMX_EventCmdComplete:
        {
...
B2GACodec::ExecutingState::ExecutingState(B2GACodec *codec)
    : BaseState(codec),
      mActive(false) {
}

B2GACodec::BaseState::PortMode B2GACodec::ExecutingState::getPortMode(
        OMX_U32 portIndex) {
    return RESUBMIT_BUFFERS;
}

void B2GACodec::ExecutingState::submitOutputBuffers() {
...
void B2GACodec::ExecutingState::resume() {
    if (mActive) {
...
void B2GACodec::ExecutingState::stateEntered() {
    ALOGV("[%s] Now Executing", mCodec->mComponentName.c_str());

    mCodec->processDeferredMessages();
}

bool B2GACodec::ExecutingState::onMessageReceived(const sp<AMessage> &msg) {
    bool handled = false;
...
status_t B2GACodec::setParameters(const sp<AMessage> &params) {
...
void B2GACodec::onSignalEndOfInputStream() {
    sp<AMessage> notify = mNotify->dup();
    notify->setInt32("what", B2GACodec::kWhatSignaledInputEOS);
...
bool B2GACodec::ExecutingState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
...
B2GACodec::OutputPortSettingsChangedState::OutputPortSettingsChangedState(
        B2GACodec *codec)
    : BaseState(codec) {
}

B2GACodec::BaseState::PortMode B2GACodec::OutputPortSettingsChangedState::getPortMode(
        OMX_U32 portIndex) {
    if (portIndex == kPortIndexOutput) {
        return FREE_BUFFERS;
    }
...
bool B2GACodec::OutputPortSettingsChangedState::onMessageReceived(
        const sp<AMessage> &msg) {
    bool handled = false;
...
void B2GACodec::OutputPortSettingsChangedState::stateEntered() {
    ALOGV("[%s] Now handling output port settings change",
         mCodec->mComponentName.c_str());
}

bool B2GACodec::OutputPortSettingsChangedState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
        case OMX_EventCmdComplete:
        {
...
B2GACodec::ExecutingToIdleState::ExecutingToIdleState(B2GACodec *codec)
    : BaseState(codec),
      mComponentNowIdle(false) {
}

bool B2GACodec::ExecutingToIdleState::onMessageReceived(const sp<AMessage> &msg) {
    bool handled = false;
...
void B2GACodec::ExecutingToIdleState::stateEntered() {
...
bool B2GACodec::ExecutingToIdleState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
        case OMX_EventCmdComplete:
        {
...
void B2GACodec::ExecutingToIdleState::changeStateIfWeOwnAllBuffers() {
...
void B2GACodec::ExecutingToIdleState::onInputBufferFilled(
        const sp<AMessage> &msg) {
...
void B2GACodec::ExecutingToIdleState::onOutputBufferDrained(
        const sp<AMessage> &msg) {
...
B2GACodec::IdleToLoadedState::IdleToLoadedState(B2GACodec *codec)
    : BaseState(codec) {
}

bool B2GACodec::IdleToLoadedState::onMessageReceived(const sp<AMessage> &msg) {
    bool handled = false;
...
void B2GACodec::IdleToLoadedState::stateEntered() {
    ALOGV("[%s] Now Idle->Loaded", mCodec->mComponentName.c_str());
}

bool B2GACodec::IdleToLoadedState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
        case OMX_EventCmdComplete:
        {
...
B2GACodec::FlushingState::FlushingState(B2GACodec *codec)
    : BaseState(codec) {
}

void B2GACodec::FlushingState::stateEntered() {
    ALOGV("[%s] Now Flushing", mCodec->mComponentName.c_str());

    mFlushComplete[kPortIndexInput] = mFlushComplete[kPortIndexOutput] = false;
}

bool B2GACodec::FlushingState::onMessageReceived(const sp<AMessage> &msg) {
    bool handled = false;
...
bool B2GACodec::FlushingState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
...
void B2GACodec::FlushingState::onOutputBufferDrained(const sp<AMessage> &msg) {
    BaseState::onOutputBufferDrained(msg);

    changeStateIfWeOwnAllBuffers();
}

void B2GACodec::FlushingState::onInputBufferFilled(const sp<AMessage> &msg) {
    BaseState::onInputBufferFilled(msg);

    changeStateIfWeOwnAllBuffers();
}

void B2GACodec::FlushingState::changeStateIfWeOwnAllBuffers() {
    if (mFlushComplete[kPortIndexInput]
            && mFlushComplete[kPortIndexOutput]
